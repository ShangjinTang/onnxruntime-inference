...
int main() {
    ...
    OrtApiBase* (*GetApiBase)() = reinterpret_cast<OrtApiBase* (*)()>(dlsym(ort_library, "OrtGetApiBase"));
    if (!GetApiBase) {
        dlclose(ort_library);
        return 1;
    }
    ...
    if (!Ort::Global<void>::api_) {
        std::cerr << "Failed to int api" << std::endl;
        return 1;
    }
    ...
    env.release();
    std::cout << "onnxruntime session/env resources released" << std::endl;

    int result = dlclose(ort_library);
    if (result != 0) {
        std::cerr << "Failed to call dlclose()" << std::endl;
        return 1;
    }
    std::cout << "onnxruntime lib closed" << std::endl;
